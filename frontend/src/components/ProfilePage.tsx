...
export function ProfilePage({ onNavigate }: ProfilePageProps) {
  ...
  const [ratingScore, setRatingScore] = useState<"positive" | "negative">(
    "positive"
  );
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        setWatchlist(
          (await axios.get("/api/bidder/watchlist", { headers })).data
        );
      } catch (e) {}
      try {
        setRawMyBids(
      ...
      if (user?.user_type === "seller") {
        try {
          setMyProducts(
            (await axios.get("/api/seller/my-products", { headers })).data
              .products
          );
        } catch (e) {}
      }
  ...
  const soldProducts = myProducts.filter(
    (p) => new Date(p.end_at).getTime() <= now && p.current_highest_bidder_id
  );
  ...
  const handleRateSeller = async () => {
    ...
    try {
      await axios.post(
        `/api/bidder/products/${ratingProduct.id}/rate`,
  ...
  const handleChangePassword = async () => {
    try {
      await axios.put(
        ...
        { oldPass: passForm.oldPass, newPass: passForm.newPass },
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    <div className="min-h-screen bg-[#F0F2F5] py-8 px-4 font-sans text-gray-900">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 flex flex-col md:flex-row items-center gap-6">
          <div
            className={`w-24 h-24 ${avatarBg} text-white rounded-full flex items-center justify-center text-4xl font-bold shadow-md border-4 border-white shrink-0`}
          >
            {avatarInitial}
          </div>
          <div className="flex-1 text-center md:text-left space-y-1">
            ...
            <div className="flex justify-center md:justify-start gap-4 text-gray-500 text-sm">
              ...
              <span className="flex items-center gap-1">
                <User className="w-4 h-4" /> ID: {user?.id}
              </span>
            </div>
            <div className="mt-2">
        ...
        <Tabs defaultValue="bids" className="w-full">
          <div className="bg-white p-1 rounded-xl shadow-sm border border-gray-200 mb-6 overflow-x-auto">
            <TabsList className="bg-transparent p-0 w-full flex justify-start gap-1 min-w-max">
              <TabsTrigger
                value="won"
                className="data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 px-4 py-2 gap-2"
              >
              ...
              <TabsTrigger
                ...
                className="data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 px-4 py-2 gap-2"
              >
                <Gavel className="w-4 h-4" /> Đang đấu
              </TabsTrigger>
              <TabsTrigger
                value="watchlist"
                className="data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 px-4 py-2 gap-2"
              ...
              <TabsTrigger
                ...
                className="data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 px-4 py-2 gap-2"
              >
                <Star className="w-4 h-4" /> Đánh giá về tôi
              </TabsTrigger>
              <TabsTrigger
                value="settings"
                className="data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 px-4 py-2 gap-2"
          ...
          <TabsContent value="won" className="outline-none">
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full min-w-[700px]">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-[40%]">
                        Sản phẩm
                      </th>
                      <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-[20%]">
                        Người bán
                      </th>
                      <th className="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase tracking-wider w-[15%]">
                        Giá chốt
                      </th>
                      <th className="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase tracking-wider w-[25%]">
                        Thao tác
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {wonBids.length > 0 ? (
                      paginate(wonBids, pageWon).map((p) => (
                        <tr
                          key={p.id}
                          ...
                          <td className="px-6 py-4 align-middle text-center">
                            ...
                            p.transaction_status === "completed" ? (
                              <Button
                                size="sm"
                                variant="outline"
                                className="h-8 border-yellow-400 text-yellow-700"
              ...
              {wonBids.length > ITEMS_PER_PAGE && (
                <PaginationBar
                  currentPage={pageWon}
                  totalItems={wonBids.length}
                  onPageChange={setPageWon}
                />
              )}
            </div>
          </TabsContent>
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <div className="space-y-5">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      className="h-10 border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                  ...
                  <Button
                    ...
                    className="w-full mt-2 h-10 border border-red-200 bg-white text-red-600 hover:bg-red-50 font-medium"
                  >
                    Lưu thay đổi
                  </Button>
                </div>
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <div className="space-y-5">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      type="password"
                      className="h-10 border border-gray-300 shadow-sm focus:border-red-500 focus:ring-1 focus:ring-red-100"
          ...
          {user?.user_type === "seller" && (
            <TabsContent
              value="my-products"
              className="outline-none space-y-12"
            >
              <div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                      ...
                      <div className="flex gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-green-600 font-bold mt-1">
                            Chốt: ${Number(p.current_price).toLocaleString()}
                          </p>
                          <p className="text-xs text-gray-500 mt-1">
                            Người thắng:{" "}
                            <span className="font-medium text-gray-800">
                              {p.bidder_name || "Ẩn danh"}
                            </span>
                          </p>
                        </div>
                      </div>
                  ...
                  {soldProducts.length === 0 && (
                    <p className="col-span-full text-center text-gray-500 py-8 bg-gray-50 rounded-xl">
                      Chưa có sản phẩm nào bán thành công.
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative border border-gray-100 animate-in fade-in zoom-in duration-200 m-4">
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-4">
                    <div
                      onClick={() => setRatingScore("positive")}
                    ...
                    >
                      <ThumbsUp className="w-8 h-8" /> Tích cực
                    </div>
                    <div
                      onClick={() => setRatingScore("negative")}
                      className={`flex-1 border-2 p-4 rounded-xl cursor-pointer transition-all flex flex-col items-center justify-center gap-2 font-bold ${
                    ...
                    >
                      <ThumbsDown className="w-8 h-8" /> Tiêu cực
                    </div>
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <Textarea
                    placeholder="Người bán có nhiệt tình không?..."
                    value={ratingComment}
                    onChange={(e) => setRatingComment(e.target.value)}
                    className="min-h-[120px] text-base p-3 resize-none border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                  />
                </div>
              </div>
              <div className="p-5 bg-gray-50 border-t flex justify-end gap-3">
                ...
                <Button
                  ...
                  className="bg-[#1a73e8] hover:bg-blue-600 h-11 px-6 font-bold text-black shadow-md"
                >
                  Gửi đánh giá
                </Button>
              </div>
      ...
      {transModalOpen && selectedTransProduct && (
        <TransactionModal
          ...
          product={selectedTransProduct}
          userRole={
            user?.user_type === "seller" &&
            selectedTransProduct.seller_id === user.id
              ? "seller"
              : "bidder"
          }
          onUpdate={() => {
