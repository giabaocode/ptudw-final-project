      ...
      <div className="max-w-6xl mx-auto">
        ...
        <Tabs defaultValue="watchlist" className="space-y-6">
          <TabsList className="bg-white p-1 rounded-xl shadow-sm inline-flex h-auto">
            <TabsTrigger value="watchlist" className="py-2 px-4">
              Đang theo dõi ({watchlist.length})
            </TabsTrigger>
            <TabsTrigger value="bids" className="py-2 px-4">
              Đang đấu giá ({myBids.length})
            </TabsTrigger>
            {user?.user_type === "seller" && (
              <TabsTrigger value="my-products" className="py-2 px-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {myBids.map((p) => (
                <ProductCard
                  ...
                  category={"Đã đặt giá"}
                  // Ưu tiên p.image trả về từ API, nếu không có thì dùng placeholder
                  image={
                    p.image || "https://placehold.co/600x400?text=No+Image"
                  }
                  onViewDetails={(id) => onNavigate("auction", id)}
                />
              ))}
